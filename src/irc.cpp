...
// Copyright (c) 2011-2012 DemonCoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("DemonCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #DemonCoinTEST0\r");
            Send(hSocket, "WHO #DemonCoinTEST0\r");
        } else {
            // randomly join #DemonCoin00-#DemonCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #DemonCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #DemonCoin%02d\r", channel_number).c_str());
        }
